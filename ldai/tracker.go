...
// TokenUsage represents the token usage returned by a model provider for a specific request. It is
// a tracking value generated by the SDK, not part of the AI Config received from LaunchDarkly.
...
// AIMetrics contains the metrics for a single AI operation, derived from an operation result by
// TrackMetricsOf or returned directly to TrackRequest. It mirrors the LDAIMetrics type in the other
// LaunchDarkly AI SDKs and is a tracking value generated by the SDK, not part of the AI Config.
type AIMetrics struct {
	...
	// ToolCalls is the ordered list of tool keys invoked during the operation; empty or nil if none
	// were observed.
	ToolCalls []string
...
func (t *Tracker) TrackTokens(usage TokenUsage) error {
	...
	// Each leaf metric error is returned (joined), not logged here; callers that swallow the error
	// — such as the TrackMetricsOf wrapper — log it via logTrackErr.
	var errs []error
	if usage.Total > 0 {
		if err := t.events.TrackMetric(tokenTotal, t.context, float64(usage.Total), t.trackData); err != nil {
			errs = append(errs, fmt.Errorf("total tokens: %w", err))
		}
	}
	if usage.Input > 0 {
		if err := t.events.TrackMetric(tokenInput, t.context, float64(usage.Input), t.trackData); err != nil {
			errs = append(errs, fmt.Errorf("input tokens: %w", err))
		}
	}
	if usage.Output > 0 {
		if err := t.events.TrackMetric(tokenOutput, t.context, float64(usage.Output), t.trackData); err != nil {
			errs = append(errs, fmt.Errorf("output tokens: %w", err))
		}
	}
...
func (t *Tracker) TrackToolCalls(toolKeys []string) error {
	...
	for _, toolKey := range toolKeys {
		if err := t.TrackToolCall(toolKey); err != nil {
			errs = append(errs, fmt.Errorf("tool call %q: %w", toolKey, err))
		}
	}
	return errors.Join(errs...)
}
...
// logTrackErr logs a delivery failure returned by a leaf Track* method that a wrapper
// (TrackMetricsOf, TrackRequest) swallows. The leaf methods return their delivery errors without
// logging them, so a direct caller can handle them; the wrappers, which discard the error rather
// than fail the whole operation, log it here instead.
...
func TrackMetricsOf[T any](
	...
	if metrics.Tokens.Set() {
		t.logTrackErr("token usage", t.TrackTokens(metrics.Tokens))
	}

	if len(metrics.ToolCalls) > 0 {
		t.logTrackErr("tool calls", t.TrackToolCalls(metrics.ToolCalls))
	}
...
// TrackMetricsOf for tasks that work with the tracker's AI Config: the task receives the current AI
// Config and returns the AIMetrics it produced (all fields optional).
//
// The request's duration is always tracked, using AIMetrics.Duration when non-zero and an
// automatically measured duration otherwise. If the task returns an error, an unsuccessful
// generation is tracked and the error is returned with zero AIMetrics. Otherwise success or error is
// tracked per AIMetrics.Success, along with any time to first token, token usage, and tool calls.
//
...
func (t *Tracker) TrackRequest(task func(c *Config) (AIMetrics, error)) (AIMetrics, error) {
	metrics, err := TrackMetricsOf(t, func(m AIMetrics) *AIMetrics {
		return &m
	}, func() (AIMetrics, error) {
