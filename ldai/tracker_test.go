...
func makeTrackData(configKey, variationKey string, version int, config *Config, runId string, graphKey string) ldvalue.Value {
	return makeTrackDataWithModel(configKey, variationKey, version, "", 1, config, runId, graphKey)
}

func makeTrackDataWithModel(
	configKey, variationKey string, version int, modelKey string, modelVersion int,
	config *Config, runId string, graphKey string,
) ldvalue.Value {
...
func TestTracker_ResumptionToken(t *testing.T) {
	...
	t.Run("includes graphKey when set", func(t *testing.T) {
		events := newMockEvents()
		config := &Config{}
		tracker := newTracker(events, newRunID(), "my-config", "var-1", 3, "", 1, ldcontext.New("key"), config, nil, "my-graph")
...
func TestTracker_TrackDataIncludesModelKeyAndVersion(t *testing.T) {
	t.Run("includes modelKey and modelVersion when set", func(t *testing.T) {
		events := newMockEvents()
		config := &Config{}
		tracker := newTracker(events, newRunID(), "key", "var", 1, "my-model", 2, ldcontext.New("key"), config, nil, "")
		assert.NoError(t, tracker.TrackSuccess())
...
func TestTracker_GraphKey_InTrackData(t *testing.T) {
	events := newMockEvents()
	config := &Config{}
	tracker := newTracker(events, newRunID(), "key", "variationKey", 1, "", 1, ldcontext.New("key"), config, nil, "my-graph")
...
func TestTrackerFromResumptionToken_GraphKey(t *testing.T) {
	events := newMockEvents()
	config := &Config{}
	original := newTracker(events, newRunID(), "my-config", "var-1", 5, "", 1, ldcontext.New("key"), config, nil, "my-graph")
...
func TestTracker_GetTrackData(t *testing.T) {
	t.Run("all fields populated from constructor args", func(t *testing.T) {
		...
		config := AICompletionConfig{aiConfigBase: aiConfigBase{model: ModelConfig{Name: "gpt-4"}, provider: ProviderConfig{Name: "openai"}}}
		tracker := newTracker(
			events, "fixed-run-id", "my-config", "var-1", 3, "my-model", 2, ldcontext.New("key"), &config, nil, "my-graph")
		...
		assert.Equal(t, "my-model", td.ModelKey)
		assert.Equal(t, 2, td.ModelVersion)
	...
	t.Run("empty optional fields when not set", func(t *testing.T) {
		events := newMockEvents()
		config := &Config{}
		tracker := newTracker(events, newRunID(), "key", "", 1, "", 1, ldcontext.New("key"), config, nil, "")

		td := tracker.GetTrackData()

		assert.Empty(t, td.VariationKey)
		assert.Empty(t, td.ModelKey)
		assert.Equal(t, 1, td.ModelVersion)
...
func TestExplicitVersionZeroInResumptionToken(t *testing.T) {
	...
	tracker := newTracker(events, newRunID(), "key", "var", 0, "", 1, ldcontext.New("user"), &config, nil, "")
...
func TestTrackDurationOf(t *testing.T) {
	t.Run("tracks measured duration and returns nil error", func(t *testing.T) {
		...
		config := &Config{}
		tracker := newTrackerWithStopwatch(
			events, newRunID(), "key", "variationKey", 1, "", 1, ldcontext.New("key"),
...
func TestTrackMetricsOf(t *testing.T) {
	...
	t.Run("judge tracker: works without completion-only rejection", func(t *testing.T) {
		...
		tracker := newTracker(events, newRunID(), "key", "variationKey", 1, "", 1, ldcontext.New("key"), judgeConfig, nil, "")
